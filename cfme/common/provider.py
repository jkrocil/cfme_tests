...
class BaseProvider(Taggable, Updateable, SummaryMixin, Navigatable):
    ...
    def add_provider_type(cls, nclass):
        cls.provider_types[nclass.type] = nclass
        return nclass
    ...
    def exists(self):
        ...
        Note:
            If provider exists but with a different name, name is updated
        """
        for prov in store.current_appliance.managed_providers:
            self.sync()
            if self.name != prov.name:
                logger.info("Provider {} already exists with name {} instead of {}; "
                            "updating name to {}"
                            .format(self.key, prov.name, self.name, prov.name))
                self.name = prov.name
            return True
        return False
